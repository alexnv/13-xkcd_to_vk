...
def upload_file(url, filename):
    ...
    if result:
        return result['photo']['owner_id'], result['photo']['id'], result['server'], result['hash']

    return None


def save_photo_to_wall(photo, server, photo_hash, vk_groupid, vk_accesstoken):
...
def publish_photo(photo_owner, photo_id, photo_comment, vk_groupid, vk_accesstoken):
...
def upload_photo_to_vk(filename, photo_comment, vk_groupid, vk_accesstoken):
    ...
    if not upload_url:
        return False
    photo_owner, photo_id, server, photo_hash = upload_file(upload_url, filename, vk_groupid, vk_accesstoken)
    if not photo_id:
        return False
    vk_saved_photo = save_photo_to_wall(photo_owner, photo_id, server, photo_hash, vk_groupid, vk_accesstoken)
...
def main():
    ...
    try:
        ...
        if not comix_filename:
            return
        comix_upload = upload_photo_to_vk(comix_filename, comix_comment, vk_accesstoken=vk_access_token,
